...
def get_result():
    ...
    for line in file:
        if 'param' in line:
            searchObj = re.search(r'(\'-acks\', \'(.*?)\', \'-batch.size\', \'(.*?)\', \'-buffer.memory\', \'(.*?)\', \'-compression.type\', \'(.*?)\', \'-linger.ms\', \'(.*?)\', \'-num.io.threads\', \'(.*?)\', \'-num.network.threads\', \'(.*?)\', \'-num.replica.fetchers\', \'(.*?)\', \'-queued.max.requests\', \'(.*?)\', \'-socket.receive.buffer.bytes\', \'(.*?)\', \'-socket.request.max.bytes\', \'(.*?)\', \'-socket.send.buffer.bytes\', \'(.*?)\')',line)
            dump_csv(searchObj.group(8)+','+searchObj.group(7)+','+searchObj.group(10)+','+searchObj.group(9)+','+searchObj.group(11)+','+searchObj.group(13)+','+searchObj.group(12)+','+searchObj.group(4)+','+searchObj.group(3)+','+searchObj.group(6)+','+searchObj.group(5), log_path='./out/result.csv')
def extract_result():
